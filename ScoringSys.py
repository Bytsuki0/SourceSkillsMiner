...
def compute_sentiment_score(username: str, token: str,
    ...
    return {'score': scr, 'details': sentiments}


def compute_oss_score(username: str, token: str, num_events: int = 1000000) -> Dict:
    """Computes OSS area score. All external calls are individually guarded."""

    try:
        opened = f1.get_user_opened_issues_and_prs(username, num_events)
        if not isinstance(opened, dict):
            print(f"Warning: get_user_opened_issues_and_prs returned unexpected type: {type(opened)}")
            opened = {'issues': [], 'prs': []}
    except Exception as e:
        print(f"Warning: could not fetch opened issues/PRs: {e}")
        opened = {'issues': [], 'prs': []}

    try:
        resolved = f1.get_user_resolved_issues_and_prs(username, num_events)
        if not isinstance(resolved, dict):
            print(f"Warning: get_user_resolved_issues_and_prs returned unexpected type: {type(resolved)}")
            resolved = {'resolved_issues': [], 'closed_prs': [], 'merged_prs_count': 0}
    except Exception as e:
        print(f"Warning: could not fetch resolved issues/PRs: {e}")
        resolved = {'resolved_issues': [], 'closed_prs': [], 'merged_prs_count': 0}

    try:
        total_commits = f1.get_commit_stats_total(username)
        # Guard: function may return a list/None/dict on API failure
        if not isinstance(total_commits, (int, float)):
            print(f"Warning: get_commit_stats_total returned unexpected type: {type(total_commits)} — defaulting to 0")
            total_commits = 0
    except Exception as e:
    ...
    # Guard merged_prs_count too — same API can misbehave
...
def compute_commit_score(username: str, token: str) -> Dict:
    ...
    if summary is None:
        return {
            'score': 0.0,
            'details': {'error': "Missing 'summary' section in analysis result.", 'criteria_met': {}}
        }
...
def get_language_usage_data(username: str, token: str) -> Dict:
    try:
        ...
        return {'error': str(e), 'languages': {}, 'language_count': 0, 'total_commits': 0, 'total_lines': 0}
